...
Status SplitPackedQKVWithRotaryEmbeddingProgram::GenerateShaderCode(ShaderHelper& sh) const {
  ...
  sh.MainFunctionBody()
      << "  // Dispatch: batch * seq * num_heads * (half_rotary_dim_vec + need_copy_dim_vec)\n"
      << "  // All sizes in uniforms are already vectorized (divided by components)\n"
      ...
      << "  let in_head_vec_idx = remainder2 % work_per_head;\n"
      << "\n"
      << "  // Calculate base offset in packed_qkv for this token (vectorized)\n"
      ...
      << "  }\n"
      << "\n"
      << "  if (in_head_vec_idx < uniforms.half_rotary_dim) {\n"
      << "    // Process rotary embedding for vectorized elements\n"
      << "    // For non-interleaved: i and j are separated by half_rotary_dim\n"
      << "    // in_head_vec_idx represents vectorized index, need scalar index for cos/sin lookup\n"
      << "    \n"
      << "    // Read vectorized Q pair (i position)\n"
      << "    let q_base = base_offset + head_idx * uniforms.head_size;\n"
      << "    let q_i_vec_offset = q_base + in_head_vec_idx;\n"
      << "    let q_j_vec_offset = q_base + uniforms.half_rotary_dim + in_head_vec_idx;\n"
      << "    let q_i_vec = " << packed_qkv.GetByOffset("q_i_vec_offset") << ";\n"
      << "    let q_j_vec = " << packed_qkv.GetByOffset("q_j_vec_offset") << ";\n"
      << "    \n"
      << "    // Apply rotary embedding component-wise\n"
      << "    // Each vector element needs its own cos/sin value\n"
      << "    var q_re_vec: packed_qkv_value_t;\n"
      << "    var q_im_vec: packed_qkv_value_t;\n"
      << "    for (var comp: u32 = 0u; comp < " << packed_qkv.NumComponents() << "u; comp++) {\n"
      << "      let scalar_idx = in_head_vec_idx * " << packed_qkv.NumComponents() << "u + comp;\n"
      ...
      << "    }\n"
      << "    \n"
      << "    let q_out_idx = batch_idx * uniforms.sequence_length * uniforms.num_heads * uniforms.head_size + seq_idx * uniforms.num_heads * uniforms.head_size + head_idx * uniforms.head_size;\n"
      << "    query[q_out_idx + in_head_vec_idx] = q_re_vec;\n"
      << "    query[q_out_idx + uniforms.half_rotary_dim + in_head_vec_idx] = q_im_vec;\n"
      << "\n"
      << "    // Process K and V if within kv_num_heads\n"
      << "    if (head_idx < uniforms.kv_num_heads) {\n"
      << "      let k_base = base_offset + uniforms.hidden_size + head_idx * uniforms.head_size;\n"
      << "      let k_i_vec_offset = k_base + in_head_vec_idx;\n"
      << "      let k_j_vec_offset = k_base + uniforms.half_rotary_dim + in_head_vec_idx;\n"
      << "      let k_i_vec = " << packed_qkv.GetByOffset("k_i_vec_offset") << ";\n"
      << "      let k_j_vec = " << packed_qkv.GetByOffset("k_j_vec_offset") << ";\n"
      << "      \n"
      << "      var k_re_vec: packed_qkv_value_t;\n"
      << "      var k_im_vec: packed_qkv_value_t;\n"
      << "      for (var comp: u32 = 0u; comp < " << packed_qkv.NumComponents() << "u; comp++) {\n"
      << "        let scalar_idx = in_head_vec_idx * " << packed_qkv.NumComponents() << "u + comp;\n"
      << "        let cos_v = " << cos_cache.GetByIndices("vec2<u32>(position_id, scalar_idx)") << ";\n"
      << "        let sin_v = " << sin_cache.GetByIndices("vec2<u32>(position_id, scalar_idx)") << ";\n"
      ...
      << "      }\n"
      << "      \n"
      << "      let k_out_idx = batch_idx * uniforms.sequence_length * uniforms.kv_num_heads * uniforms.head_size + seq_idx * uniforms.kv_num_heads * uniforms.head_size + head_idx * uniforms.head_size;\n"
      << "      key[k_out_idx + in_head_vec_idx] = k_re_vec;\n"
      << "      key[k_out_idx + uniforms.half_rotary_dim + in_head_vec_idx] = k_im_vec;\n"
      << "\n"
      << "      // V doesn't need rotary, just copy both vector positions\n"
      << "      let v_base = base_offset + uniforms.hidden_size + uniforms.kv_hidden_size + head_idx * uniforms.head_size;\n"
      << "      let v_i_vec = " << packed_qkv.GetByOffset("v_base + in_head_vec_idx") << ";\n"
      << "      let v_j_vec = " << packed_qkv.GetByOffset("v_base + uniforms.half_rotary_dim + in_head_vec_idx") << ";\n"
      << "      let v_out_idx = batch_idx * uniforms.sequence_length * uniforms.kv_num_heads * uniforms.head_size + seq_idx * uniforms.kv_num_heads * uniforms.head_size + head_idx * uniforms.head_size;\n"
      << "      val[v_out_idx + in_head_vec_idx] = v_i_vec;\n"
      << "      val[v_out_idx + uniforms.half_rotary_dim + in_head_vec_idx] = v_j_vec;\n"
      << "    }\n"
      << "  } else {\n"
      << "    // Process non-rotary elements (direct copy, already vectorized)\n"
      << "    let actual_vec_idx = uniforms.half_rotary_dim + in_head_vec_idx;\n"
      << "\n"
      << "    // Copy Q\n"
      << "    let q_offset = base_offset + head_idx * uniforms.head_size + actual_vec_idx;\n"
      << "    let q_data = " << packed_qkv.GetByOffset("q_offset") << ";\n"
      << "    let q_out_idx = batch_idx * uniforms.sequence_length * uniforms.num_heads * uniforms.head_size + seq_idx * uniforms.num_heads * uniforms.head_size + head_idx * uniforms.head_size + actual_vec_idx;\n"
      << "    query[q_out_idx] = q_data;\n"
      << "\n"
      << "    // Copy K and V if within kv_num_heads\n"
      << "    if (head_idx < uniforms.kv_num_heads) {\n"
      << "      let k_offset = base_offset + uniforms.hidden_size + head_idx * uniforms.head_size + actual_vec_idx;\n"
      << "      let k_data = " << packed_qkv.GetByOffset("k_offset") << ";\n"
      << "      let k_out_idx = batch_idx * uniforms.sequence_length * uniforms.kv_num_heads * uniforms.head_size + seq_idx * uniforms.kv_num_heads * uniforms.head_size + head_idx * uniforms.head_size + actual_vec_idx;\n"
      << "      key[k_out_idx] = k_data;\n"
      << "\n"
      << "      let v_offset = base_offset + uniforms.hidden_size + uniforms.kv_hidden_size + head_idx * uniforms.head_size + actual_vec_idx;\n"
      << "      let v_data = " << packed_qkv.GetByOffset("v_offset") << ";\n"
      << "      let v_out_idx = batch_idx * uniforms.sequence_length * uniforms.kv_num_heads * uniforms.head_size + seq_idx * uniforms.kv_num_heads * uniforms.head_size + head_idx * uniforms.head_size + actual_vec_idx;\n"
      << "      val[v_out_idx] = v_data;\n"
      << "    }\n"
      << "  }\n";
